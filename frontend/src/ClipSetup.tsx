...
export type ClipSetupProps = {
  ...
  /** Tour-only reveal; it never changes or persists the user's wizard step. */
  tourStep?: ClipSetupStep;
};

export type ClipSetupStep =
  "source" | "moments" | "camera" | "captions" | "review";
type Step = ClipSetupStep;
...
function mergeSettings(
  initial?: Partial<ClipSetupSettings>,
): ClipSetupSettings {
  return {
    ...defaultSettings,
    ...initial,
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        rangeEnd:
          stored.rangeEnd ??
          String(Math.min(30, Math.max(1, Math.round(duration)))),
      };
    }
  } catch {
...
export default function ClipSetup({
  ...
  const [draft] = useState(() =>
    readDraft(project.id, project.duration, initialSettings),
  );
  ...
  useEffect(() => {
    ...
    fetch(`/api/projects/${project.id}/readiness`, {
      signal: controller.signal,
    })
      .then(async (response) => {
        if (!response.ok)
          throw new Error("Could not check this source. Reload the project.");
  ...
  const displayStep =
    tourStep && steps.some((item) => item.id === tourStep) ? tourStep : step;
  ...
  const addRange = () => {
    ...
    update(
      "ranges",
      [...settings.ranges, { start, end }].sort((a, b) => a.start - b.start),
    );
  ...
  const removeRange = (index: number) =>
    update(
      "ranges",
      settings.ranges.filter((_, itemIndex) => itemIndex !== index),
    );
  ...
  const validateNumbers = () => {
    ...
    if (
      ...
      setError(
        "Clip length must be 5–300 seconds and maximum clips must be 1–20.",
      );
      return false;
    }
    if (
    ...
    if (
      !finite(camera.camera_zoom) ||
      camera.camera_zoom < 1 ||
      camera.camera_zoom > 1.5 ||
      !finite(camera.camera_dead_zone) ||
      camera.camera_dead_zone < 0 ||
      camera.camera_dead_zone > 0.3 ||
      !finite(camera.focus_x) ||
      camera.focus_x < 0 ||
      camera.focus_x > 1 ||
    ...
    if (
      ...
      setError(
        "Each selected moment must be inside the source and longer than zero seconds.",
      );
      return false;
    }
    return true;
  ...
  const validateStep = (currentStep: Step) => {
    ...
    if (
      ...
      setError(
        "Choose an available ranking provider, or configure it in Settings.",
      );
      return false;
    }
    if (
  ...
  return (
    <main
      className="clip-setup"
      aria-labelledby="clip-setup-title"
      ...
      <div className="clip-setup-layout">
            ...
            {steps.map(({ id, label, icon: Icon }, index) => {
              ...
              return (
                <button
                  type="button"
                  key={id}
                  className={`clip-setup-step ${displayStep === id ? "is-current" : ""} ${complete ? "is-complete" : ""}`}
                  onClick={() => {
                    if (index <= stepIndex) {
                      setError("");
                      setStep(id);
                  ...
                  aria-current={displayStep === id ? "step" : undefined}
                >
        ...
        <section className="clip-setup-stage">
          <div className="clip-setup-stage-heading">
            <div>
              <h2>
                {displayStep === "source" && "Review your source"}
                {displayStep === "moments" &&
                  "Which moments should become clips?"}
                {displayStep === "camera" && "Set the framing."}
                {displayStep === "captions" && "Set up captions"}
                {displayStep === "review" && "Review and generate"}
              </h2>
            </div>
          </div>
            ...
            <div className="clip-setup-source-grid" data-tour="setup-source">
              <div className="clip-setup-preview" aria-label="Source preview">
                {sourcePreview ? (
                  ...
                  <div className="clip-setup-preview-empty">
                    <Film size={27} />
                    <span>Preview will appear here</span>
                  </div>
                )}
                <span className="clip-setup-preview-duration">
                  {formatDuration(project.duration)}
                </span>
              </div>
              <div className="clip-setup-source-copy">
                ...
                <p>
                  Clipflow will create a new set of editable clips from this
                  source.
                </p>
                {readiness?.source_ready === false && (
                  <div className="source-recovery" role="alert">
                    <p>{readiness.message}</p>
                    <button type="button" onClick={onReplaceSource}>
                      Import source again
                    </button>
                  </div>
                )}
                <button
                  className="clip-setup-automatic"
                  type="button"
                  disabled={
                    busy ||
                    !readiness?.source_ready ||
                    readiness.speech?.ready === false
                  }
                  onClick={chooseAutomaticSetup}
                >
                  <Sparkles size={16} /> Create clips automatically
                </button>
                <small className="clip-setup-automatic-note">
                  Find moments, preserve the subject, and add captions using
                ...
                {readiness?.speech && (
                  <p className="processing-readiness" role="status">
                    {readiness.speech.ready
                      ? `Speech: ${readiness.speech.selected_model || readiness.speech.provider}.`
                      : "Speech needs more free disk space or a configured provider. Use manual moments without captions, or change speech settings."}{" "}
                    {readiness.speech.warning}
                    {readiness.speech.selected_model &&
                    !readiness.speech.cached_models?.includes(
                      readiness.speech.selected_model,
                    )
                      ? " A free model will download before the first run."
                      : ""}
                  </p>
                )}
                <dl className="clip-setup-source-meta">
                  {sourceMeta.map((item, index) => (
                    <div key={item}>
                      <dt>{["Length", "Frame", "Rate"][index]}</dt>
                      <dd>{item}</dd>
                    </div>
                  ))}
                </dl>
            ...
            <div className="clip-setup-form" data-tour="setup-moments">
              <div className="clip-setup-options clip-setup-options-wide">
                <OptionCard
                  ...
                  detail="Find clear moments around scene changes and natural pauses."
                  icon={<Sparkles size={19} />}
                  onClick={() => update("mode", "smart")}
                />
                <OptionCard
                  checked={settings.mode === "full"}
                  title="Split the whole source"
                  detail="Make a clean sequence of clips at the length you choose."
                  icon={<Scissors size={19} />}
                  onClick={() => update("mode", "full")}
                />
                <OptionCard
                  checked={settings.mode === "manual"}
                  title="Choose moments myself"
                  detail="Add exact ranges when timing matters more than discovery."
                  icon={<Focus size={19} />}
              ...
              {settings.mode !== "manual" ? (
                <div className="clip-setup-subgrid">
                  <label className="clip-setup-field">
                    <FieldLabel hint="5–300 seconds">Clip length</FieldLabel>
                    <div className="clip-setup-number">
                      <input
                        type="number"
                        min={5}
                        max={300}
                        ...
                        onChange={(event) =>
                          update(
                            ...
                            Math.max(
                              5,
                              Math.min(300, Number(event.target.value) || 5),
                            ),
                          )
                        }
                      />
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setSettings((current) => ({
                          ...
                          use_transcript: event.target.value.trim()
                            ? true
                            : current.use_transcript,
                        }))
                      }
                      placeholder="e.g. the clearest explanation"
                    />
                ...
                <div className="clip-setup-manual-ranges">
                  <div className="clip-setup-manual-source">
                    {project.source_url ? (
                      <video
                        ...
                        aria-label="Source video for marking moments"
                      />
                    ) : project.thumbnail_url ? (
                      <img src={project.thumbnail_url} alt="Source thumbnail" />
                    ) : (
                    ...
                    <div className="clip-setup-manual-source-controls">
                      ...
                      <button type="button" onClick={() => markRange("start")}>
                        Mark start
                      </button>
                      <button type="button" onClick={() => markRange("end")}>
                        Mark end
                      </button>
                    </div>
                  </div>
                  <p className="clip-setup-range-pending" role="status">
                    Pending moment:{" "}
                    <strong>
                      {formatDuration(Number(rangeStart) || 0)} –{" "}
                      {formatDuration(Number(rangeEnd) || 0)}
                    </strong>
                    . Mark both points, then add the moment.
                  </p>
                  <div className="clip-setup-range-entry">
                    ...
                    <button
                      className="clip-setup-add-range"
                      type="button"
                      onClick={addRange}
                    >
                  ...
                  {settings.ranges.length > 0 ? (
                    <ol className="clip-setup-ranges">
                    ...
                    <p className="clip-setup-empty-note">
                      Add a range from the source timeline. Each range becomes
                      one editable clip.
                    </p>
                  )}
                </div>
              ...
              {settings.mode !== "manual" && (
                <label className="clip-setup-checkline">
                  <input
                    type="checkbox"
                    checked={
                      settings.use_transcript || Boolean(settings.topic.trim())
                    }
                    disabled={Boolean(settings.topic.trim())}
                    onChange={(event) =>
                      setSettings((current) => ({
                        ...current,
                        use_transcript: event.target.checked,
                      }))
                    }
                  />
                  <span>
                    <strong>Use speech when ranking</strong>
                    <small>
                      {settings.topic.trim()
                        ? "On because you added a topic. Transcription runs when you generate."
                        : "Optional. Turn it on when spoken words should guide the highlights."}
                    </small>
                  </span>
                </label>
              )}
                ...
                <>
                  <label className="clip-setup-field clip-setup-provider">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        update(
                          "provider",
                          event.target.value as AnalysisOptions["provider"],
                        )
                      }
                    >
                      {!providerAvailable && (
                  ...
                  <div className="clip-setup-subgrid clip-setup-speech-options">
                    <label className="clip-setup-field">
                      ...
                      <select
                        value={settings.captions.language}
                        onChange={(event) =>
                          updateCaptions("language", event.target.value)
                        }
                      >
                    ...
                    <label className="clip-setup-field">
                      <FieldLabel>Transcription quality</FieldLabel>
                      <select
                        value={settings.captions.quality}
                        onChange={(event) =>
                          updateCaptions(
                            "quality",
                            event.target
                              .value as ClipSetupSettings["captions"]["quality"],
                          )
                        }
                      >
                        <option value="auto">
                          Automatic · choose an available model
                        </option>
                    ...
                    <label className="clip-setup-field">
                      ...
                      <select
                        value={settings.captions.dialect}
                        onChange={(event) =>
                          updateCaptions(
                            "dialect",
                            event.target
                              .value as ClipSetupSettings["captions"]["dialect"],
                          )
                        }
                      >
                        <option value="none">None</option>
                        <option value="algerian">Algerian Darija</option>
                      </select>
                    </label>
                  </div>
                </>
          ...
          {displayStep === "camera" && (
            <div className="clip-setup-form" data-tour="setup-camera">
              <div className="clip-setup-options clip-setup-options-wide">
                <OptionCard
                  checked={settings.camera.camera_strategy === "adaptive"}
                  title="Adaptive framing"
                  detail="Preserve people, diagrams and the active subject as the crop changes."
                  icon={<Sparkles size={19} />}
                  onClick={() => {
                    updateCamera("camera_strategy", "adaptive");
                    updateCamera("framing", "follow");
                  }}
                />
                <OptionCard
                  checked={settings.camera.camera_strategy === "follow"}
                  title="Follow subjects"
                  detail="Keep the active subject in frame with a smooth crop."
                  icon={<Focus size={19} />}
                  onClick={() => {
                    updateCamera("camera_strategy", "follow");
                    updateCamera("framing", "follow");
                  }}
                />
                <OptionCard
                  checked={settings.camera.camera_strategy === "manual"}
                  title="Manual crop"
                  detail="Keep a fixed crop; add camera keyframes when editing a clip."
                  icon={<Film size={19} />}
                  onClick={() => {
                    updateCamera("camera_strategy", "manual");
                    updateCamera("framing", "manual");
                  }}
                />
              </div>
              <div className="clip-setup-subgrid">
                <label className="clip-setup-field">
                  ...
                  <select
                    value={settings.camera.aspect_ratio}
                    onChange={(event) =>
                      updateCamera(
                        "aspect_ratio",
                        event.target
                          .value as ClipSetupSettings["camera"]["aspect_ratio"],
                      )
                    }
                  >
                ...
                <label className="clip-setup-field">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateCamera("resolution", Number(event.target.value))
                    }
                  >
                ...
                {settings.camera.camera_strategy !== "manual" && (
                  <label className="clip-setup-field">
                    <FieldLabel>Subject position</FieldLabel>
                    <select
                      value={settings.camera.subject}
                      onChange={(event) =>
                        updateCamera(
                          "subject",
                          event.target
                            .value as ClipSetupSettings["camera"]["subject"],
                        )
                      }
                    >
                ...
                {settings.camera.camera_strategy !== "manual" && (
                  <label className="clip-setup-field">
                    <FieldLabel>Motion preset</FieldLabel>
                    <select
                      value={settings.camera.camera_motion}
                      onChange={(event) =>
                        updateCamera(
                          "camera_motion",
                          event.target
                            .value as ClipSetupSettings["camera"]["camera_motion"],
                        )
                      }
                    >
                ...
                {settings.camera.camera_strategy === "adaptive" && (
                  <label className="clip-setup-field">
                    <FieldLabel hint="Used when the subject is hard to track">
                      Safety framing
                    </FieldLabel>
                    <select
                      value={settings.camera.safe_framing}
                      onChange={(event) =>
                        updateCamera(
                          "safe_framing",
                          event.target
                            .value as ClipSetupSettings["camera"]["safe_framing"],
                        )
                      }
                    >
                      <option value="fit">Contain the whole frame</option>
                      <option value="blur">Blur the backdrop</option>
                    </select>
                  </label>
                )}
                {settings.camera.camera_strategy === "adaptive" && (
                  <label className="clip-setup-field">
                    <FieldLabel hint="Gemini is opt-in">
                      Vision provider
                    </FieldLabel>
                    <select
                      value={settings.camera.vision_provider}
                      onChange={(event) =>
                        updateCamera(
                          "vision_provider",
                          event.target
                            .value as ClipSetupSettings["camera"]["vision_provider"],
                        )
                      }
                    >
              ...
              {settings.camera.camera_strategy !== "manual" && (
                <label className="clip-setup-checkline">
                  <input
                    type="checkbox"
                    checked={settings.camera.camera_auto_zoom}
                    onChange={(event) =>
                      setSettings((current) => ({
                        ...current,
                        camera: {
                          ...current.camera,
                          camera_auto_zoom: event.target.checked,
                          camera_zoom: event.target.checked
                            ? Math.max(1.25, current.camera.camera_zoom)
                            : current.camera.camera_zoom,
                        },
                      }))
                    }
                  />
                  <span>
                    <strong>Adjust zoom to face size</strong>
                    <small>
                      Pull back when the face is close; cap the crop at your
                      maximum zoom.
                    </small>
                  </span>
                </label>
              ...
              {settings.camera.camera_strategy !== "manual" && (
                <div className="clip-setup-range-fields">
                  <label className="clip-setup-range-control">
                    <FieldLabel>
                      {settings.camera.camera_auto_zoom
                        ? "Maximum zoom"
                        : "Crop zoom"}{" "}
                      <b>{settings.camera.camera_zoom.toFixed(2)}×</b>
                    </FieldLabel>
                    <input
                      type="range"
                      min={1}
                      max={1.5}
                      step={0.01}
                      value={settings.camera.camera_zoom}
                      onChange={(event) =>
                        updateCamera("camera_zoom", Number(event.target.value))
                      }
                    />
                  </label>
                  <label className="clip-setup-range-control">
                    <FieldLabel>
                      Tracking room{" "}
                      <b>
                        {Math.round(settings.camera.camera_dead_zone * 100)}%
                      </b>
                    </FieldLabel>
                    <input
                      type="range"
                      ...
                      max={0.3}
                      step={0.01}
                      value={settings.camera.camera_dead_zone}
                      onChange={(event) =>
                        updateCamera(
                          "camera_dead_zone",
          ...
          {displayStep === "captions" && (
            <div className="clip-setup-form" data-tour="setup-captions">
              <div className="clip-setup-options clip-setup-options-wide">
                <OptionCard
                  checked={settings.captions.mode === "none"}
                  title="No captions"
                  detail="Keep the source audio and frame clean."
                  icon={<Volume2 size={19} />}
                  onClick={() => updateCaptions("mode", "none")}
                />
                <OptionCard
                  checked={settings.captions.mode === "auto"}
                  title="Auto captions"
                  detail="Transcribe spoken words into editable subtitles."
                  icon={<Sparkles size={19} />}
                  onClick={() => {
                    updateCaptions("mode", "auto");
                    update("use_transcript", true);
                  }}
                />
                <OptionCard
                  checked={settings.captions.mode === "manual"}
                  title="One manual caption"
                  detail="Add a short overlay that stays on each generated clip."
                  icon={<Captions size={19} />}
                  onClick={() => updateCaptions("mode", "manual")}
                />
              </div>
              {settings.captions.mode === "manual" && (
                <label className="clip-setup-field">
                  <FieldLabel hint="Shown on every generated clip">
                    Caption text
                  </FieldLabel>
                  <textarea
                    rows={3}
                    maxLength={4000}
                    value={settings.captions.text}
                    onChange={(event) =>
                      updateCaptions("text", event.target.value)
                    }
                    placeholder="Write a short caption…"
                  />
                </label>
              )}
              {settings.captions.mode === "auto" && (
                <div className="clip-setup-subgrid">
                  <label className="clip-setup-field">
              ...
              {settings.captions.mode !== "none" && (
                <div className="clip-setup-subgrid">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        updateCaptions(
                          "style",
                          event.target
                            .value as ClipSetupSettings["captions"]["style"],
                        )
                      }
                    >
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        updateCaptions(
                          "font",
                          event.target
                            .value as ClipSetupSettings["captions"]["font"],
                        )
                      }
                    >
                  ...
                  <label className="clip-setup-field">
                    <FieldLabel hint="32–90 px">Size</FieldLabel>
                    <input
                      type="number"
                      min={32}
                      max={90}
                      ...
                      onChange={(event) =>
                        updateCaptions(
                          ...
                          Math.max(
                            32,
                            Math.min(90, Number(event.target.value) || 52),
                          ),
                        )
                      }
                    />
                  ...
                  <label className="clip-setup-field">
                    ...
                    <div className="clip-setup-color">
                      <input
                        type="color"
                        value={settings.captions.color}
                        onChange={(event) =>
                          updateCaptions("color", event.target.value)
                        }
                      />
                      <span>{settings.captions.color}</span>
                    </div>
                  </label>
                  <label className="clip-setup-field">
                    <FieldLabel>Position</FieldLabel>
                    <select
                      value={settings.captions.y === 0.5 ? "center" : "bottom"}
                      onChange={(event) =>
                        updateCaptions(
                          "y",
                          event.target.value === "center" ? 0.5 : 0.86,
                        )
                      }
                    >
          ...
          {displayStep === "review" && (
            <div className="clip-setup-review" data-tour="setup-review">
              <div className="clip-setup-review-hero">
                <div className="clip-setup-review-mark">
                  <Check size={24} />
                </div>
                <div>
                  <h3>
                    {settings.mode === "manual"
                      ? `${settings.ranges.length} moments selected`
                      : settings.mode === "smart"
                        ? settings.use_transcript
                          ? "Highlights with speech ranking"
                          : "Structural highlights"
                        : "Full source split"}
                  </h3>
                  <p>
                    {settings.mode === "smart" && !settings.use_transcript
                      ? "Scene changes and natural pauses shape the first pass. You can edit every clip after generation."
                      : "Clipflow will make a first pass you can edit clip by clip."}
                  </p>
                </div>
              </div>
              <div className="clip-setup-review-grid">
                <div>
                  <span>Camera</span>
                  <strong>
                    {settings.camera.camera_strategy === "adaptive"
                      ? "Adaptive framing"
                      : settings.camera.camera_strategy === "follow"
                        ? "Follow subjects"
                        : "Manual crop"}
                  </strong>
                </div>
                <div>
                  ...
                  <strong>
                    {settings.captions.mode === "auto"
                      ? `Auto · ${settings.captions.language === "auto" ? "detected language" : settings.captions.language}`
                      : settings.captions.mode === "manual"
                        ? "Manual overlay"
                        : "Off"}
                  </strong>
                ...
                <div>
                  <span>Output</span>
                  <strong>
                    {settings.camera.aspect_ratio} ·{" "}
                    {settings.camera.resolution}p
                  </strong>
                </div>
                <div>
                  <span>Audio</span>
                  <strong>
                    {settings.audio.denoise ? "Denoised" : "Original audio"}
                  </strong>
                </div>
              </div>
              <details className="clip-setup-advanced">
                <summary>Audio finishing</summary>
                <div className="clip-setup-range-fields">
                  <label className="clip-setup-range-control">
                    <FieldLabel>
                      Volume <b>{Math.round(settings.audio.volume * 100)}%</b>
                    </FieldLabel>
                    <input
                      type="range"
                      min={0}
                      ...
                      onChange={(event) =>
                        updateAudio("volume", Number(event.target.value))
                      }
                    />
                  </label>
                  <label className="clip-setup-range-control">
                    <FieldLabel>
                      Fade <b>{settings.audio.fade.toFixed(1)} sec</b>
                    </FieldLabel>
                    <input
                      type="range"
                      min={0}
                      max={2}
                      step={0.1}
                      value={settings.audio.fade}
                      onChange={(event) =>
                        updateAudio("fade", Number(event.target.value))
                      }
                    />
                  </label>
                </div>
                <label className="clip-setup-checkline">
                  <input
                    type="checkbox"
                    checked={settings.audio.denoise}
                    onChange={(event) =>
                      updateAudio("denoise", event.target.checked)
                    }
                  />
                  <span>
                    <strong>Clean up background noise</strong>
                    <small>
                      Useful for interviews and handheld recordings.
                    </small>
                  </span>
                </label>
              </details>
          ...
          {error && (
            <p className="clip-setup-error" role="alert">
              {error}
            </p>
          )}
          <div className="clip-setup-actions">
            <button
              className="clip-setup-back"
              type="button"
              onClick={goBack}
              disabled={busy}
            >
            ...
            {step === "review" ? (
              ...
              <button
                className="clip-setup-next"
                type="button"
                onClick={goNext}
              >
                {step === "source" ? "Configure moments" : "Continue"}{" "}
                <ArrowRight size={16} />
              </button>
            )}
          </div>
