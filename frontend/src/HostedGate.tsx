...
async function readSession(): Promise<{ mode: 'local' | 'hosted'; authenticated: boolean; user_id?: string }> {
  const response = await fetch('/api/auth/session', {
    credentials: 'same-origin',
    cache: 'no-store',
  })
  // A missing probe is the expected signal from an unmodified local worker.
  if (response.status === 404 && !forcedHostedMode) return { mode: 'local', authenticated: false }
  if (!response.ok) throw new Error(`session endpoint returned ${response.status}`)
  const payload = await response.json() as { mode?: 'local' | 'hosted'; authenticated?: boolean; user_id?: string }
  const mode = payload.mode ?? (forcedHostedMode ? 'hosted' : 'local')
  if (forcedHostedMode && mode !== 'hosted') throw new Error('hosted session endpoint is not installed')
  if (typeof payload.authenticated !== 'boolean') throw new Error('invalid session response')
  return { mode, authenticated: payload.authenticated, user_id: payload.user_id }
}
...
function LoginPanel({ onSignedIn }: { onSignedIn: (userId: string | null) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(response.status >= 500 ? 'The hosted worker is unavailable.' : response.status === 429 ? 'Too many attempts. Try again shortly.' : 'Sign-in failed.')
        return
      }
  ...
  return (
    <main className="hosted-gate hosted-gate-login">
      <section className="hosted-login-card" aria-labelledby="hosted-login-title">
        ...
        <form onSubmit={submit}>
          <label>
            Email
            <input autoComplete="email" inputMode="email" required value={email} onChange={(event) => setEmail(event.target.value)} />
          </label>
          <label>
            Password
            <input autoComplete="current-password" required type="password" value={password} onChange={(event) => setPassword(event.target.value)} />
          </label>
          {error && <p className="hosted-form-error" role="alert">{error}</p>}
...
export function HostedGate({ children }: { children: ReactNode }) {
  const [mode, setMode] = useState<RuntimeMode>(forcedHostedMode ? 'hosted' : 'unknown')
  ...
  const logout = async () => {
    ...
    try {
      ...
      setLogoutError('Sign out failed. Check the worker connection and try again.')
    }
  }

  const value = useMemo(() => ({ hosted: mode === 'hosted', status, userId, logoutError, logout }), [mode, status, userId, logoutError])

  if (mode === 'local') {
    return <HostedAuthContext.Provider value={value}>{children}</HostedAuthContext.Provider>
  }
  if (status === 'checking') {
    return <main className="hosted-gate hosted-gate-loading"><span>Checking workspace session…</span></main>
  }
  if (status === 'signed-out') {
    return <HostedAuthContext.Provider value={value}><LoginPanel onSignedIn={(nextUserId) => { setUserId(nextUserId); setStatus('signed-in') }} /></HostedAuthContext.Provider>
  }
